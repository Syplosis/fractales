...
window_size = (300, 300)
division_window = window_size[0] // 5
shifting_coef = 10
    ...
    for x in range(x_start, x_end):
        for y in range(y_start, y_end):
            ...
            for i in range(50):
                ...
                #############################################
                if x == x_start:
                    drawable_image.point((x - gap_offset + pil_image.width // 2, y + pil_image.height // 2), (255, 0, 0, 255))
                elif abs(z) > 2:
...
def fast_image_shift(x_offset, y_offset, image):
    ...
    shifted_image = Image.new("RGBA", (width, height), (255, 255, 255, 255))
...
def move_to_the_left():
    ...
    gap += shifting_coef  # Décale l'image vers la gauche en modifiant 'gap'
    generation(gap, division_window, x_start=window_size[0] // 2 - shifting_coef, pil_image=out)
    update_image()
...
def move_to_the_right():
    ...
    generation(gap, division_window, x_end=-out.width // 2 + shifting_coef, pil_image=out)
    update_image()
